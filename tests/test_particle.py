...
def test_particle_weight_array_updates_weighted_flow_and_temperature():
    vel = np.array(
        [
            [1.0, 0.0, 0.0],
            [3.0, 0.0, 0.0],
    ...
    particle = Particle(name="X", charge=1, mass=2.0, density=float(np.sum(weights)), Nmarker=3, weight=weights, vel=vel)

    expected_flow = np.sum(vel * weights[:, np.newaxis], axis=0) / np.sum(weights)
    expected_temperature = (
        particle.mass
        / (3.0 * e)
        * (
            np.sum(weights[:, np.newaxis] * vel ** 2) / np.sum(weights)
            - np.square(expected_flow).sum()
        )
    )

    assert particle.uses_particle_weights
    assert_allclose(particle.flow_actual, expected_flow)
    assert_allclose(particle.temperature_actual, expected_temperature)
